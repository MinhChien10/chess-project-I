...
class Controller:
    ...
    def check_menu_buttons(self, button, window):
        ...
        if button == 'Change AGL':
            buttons, values = sg.Window('Select AGL',  # Create popup window for depth selection
                                        [[sg.Combo(constants.ALGORITHM_LIST, size=(10, 15), readonly=True,
                                                   k='agl_select'), sg.OK(), sg.Cancel()]],
                                        finalize=True, font=('Helvetica', 12), icon=constants.LOGO).read(close=True)
            if buttons == 'OK':
                self.depth = values['agl_select']
                window['el_algorithm'](self.depth)
